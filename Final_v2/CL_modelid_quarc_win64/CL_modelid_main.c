...
typedef RT_MODEL_CL_modelid_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * CL_modelid(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(CL_modelid_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://CL_modelid:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: CL_modelid -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
